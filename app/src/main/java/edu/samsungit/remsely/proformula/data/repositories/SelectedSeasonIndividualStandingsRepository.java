...
public class SelectedSeasonIndividualStandingsRepository {
    private final DatabaseReference databaseReference;

    public SelectedSeasonIndividualStandingsRepository(){
        databaseReference = FirebaseDatabase.getInstance().getReference();
    }

    public MutableLiveData<List<IndividualStandingsPositionDataModel>> getIndividualStandingsLiveData(String seasonsKey){
                                            ...
                                            databaseReference.child(TEAMS).child(teamKey)
                                                    .addListenerForSingleValueEvent(new ValueEventListener() {
                                                        ...
                                                        public void onDataChange(@NonNull DataSnapshot teamSnapshot) {
                                                            String teamName = teamSnapshot.child(SHORT_NAME)
                                                                    .getValue(String.class);
                                                            String teamLogo = teamSnapshot.child(LOGO_UPPER)
                                                                    .getValue(String.class);

                                                            IndividualStandingsPositionDataModel standingsPosition
                                                                    = new IndividualStandingsPositionDataModel(
                                                                    position, pilotFlag, pilotName, teamLogo, teamName, points);
                                                            standings.add(standingsPosition);
                                                            standings.sort(Comparator.comparingInt(
                                                                    IndividualStandingsPositionDataModel::getP));
